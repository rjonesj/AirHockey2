...
//		puck.setRotation((float)Math.toDegrees(puckBody.getAngle()));
	...
	private void resetWorldBodies() {
		...
		while (it.hasNext()) {
			...
			smokeMouseJoint((Integer)pair.getKey());
		}
...
//		touchBodyRight.applyForceToCenter(0,3,true);
	...
	public boolean touchDown(int screenX, int screenY, int pointer, int button) {
		...
		if(testPoint.y/PIXELS_TO_METERS < -h/2 || testPoint.y/PIXELS_TO_METERS > h/2
			...
			if (hitBody != null) {
				...
				Gdx.app.log("TD", "Put a joint in for pointer: "+pointer);
	...
	public boolean touchUp(int screenX, int screenY, int pointer, int button) {
		...
		smokeMouseJoint(pointer);

		return true;
	}

	private void smokeMouseJoint(int pointer) {
		...
		if (pointerJoint != null) {
			...
			Gdx.app.log("TD", "Mouse joint at pointer: "+pointer+" was destroyed.");
		}
